...
namespace GISUtilities
{
    public class CRailwayScene
    {
        ...
        public string mDataPath = @"C:\GISData\jiqing\";
        public string    mProjectPath = "JQMIS.CN";  // 数据存放路径与工程存放路径
        ...
        {
            //1. 初始化线路信息
            CRailwayLineList.CreateLinelistFromSqlite(mDataPath+"gisdb.db");
            ...
            if (CServerWrapper.isConnected)
            {
                ...
                if (!resultStr.Equals("登录成功"))
                    return;
                // FIXME 某个工点异常，报错，排除该工点后，继续初始化其他工点
        ...
        public CRailwayScene(string dbPath, string prjUrl)
        {
            ...
            foreach (CContBeam cb in mContBeamList)
            {
                //if (cb.mProjectID == 337 || cb.mProjectID == 402 || cb.mProjectID == 403)
                    cb.AdjustMileage();
                //else
                //    cb.AdjustMileage();
            }
...
//#endif
    ...
    ProfessionalCategoryCode like '-1-42-27-%' OR ProfessionalCategoryCode like '-1-42-28-%' order by ProjectID asc;";
            ...
            else if (fromLocal)
            {
                ...
                //dt = DatabaseWrapper.ExecuteDataTable(dbPath, @"SELECT * from ProjectInfo order by ProjectID asc;");
                //DatabaseWrapper.PrintDataTable(dt);
        ...
        private void proj2Txt(StreamWriter sw) {
            sw.WriteLine("ProjectID\tParentID\tProjectName\tProfessionalName\tProfessionalCategoryCode\tShorName\t" 
                + "Mileage_Start_Des\tMileage_End_Des\tMileage_Start\tMileage_End\tSerialNo\tUpdateTime\tDirection\t"
                + "avgProgress\tIsValid\tPhotoUrl");

            foreach (CRailwayProject p in mTotalProjectList)
            {
                sw.WriteLine(p.mProjectID + "\t" +p.mParentID + "\t" + p.ProjectName + "\t" + p.ProfessionalName
                    + "\t" + p.mProfessionalCode + "\t" + p.SegmentName + "\t" + p.Mileage_Start_Discription + "\t" + p.Mileage_End_Discription
                     + "\t" + p.mMileage_Start + "\t" + p.mMileage_End + "\t" + p.mSerialNo + "\t" + p.UpdateTime + "\t" + p.mDirection
                      + "\t" + p.AvgProgress + "\t" + p.mIsValid + "\t" + p.mPhotoUrl);
            }
            sw.Close();
        }
        ...
        private void initSomeProj(String dbPath,DataTable dt, bool fromLocal = true)
        {
            ...
            try
            {
                ...
                foreach (DataRow dataReader in dt.Rows)
                {
                    ...
                    //if (projID == 437)
                    //    Console.WriteLine("调试" + projectName);
