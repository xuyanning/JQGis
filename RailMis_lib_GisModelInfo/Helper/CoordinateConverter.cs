...
namespace ModelInfo
{
    ...
    public class CoordinateConverter
    {
        ...
        */
        ...
        {
            //int zone = (int)((lon[0] + 180.0) / 6) + 1;
            //FIXMe， 济青统一117度计算，优点是统一utm空间，缺点是青岛附近120度，误差大
            double t = UTMCentralMeridian(zoneJQ);
            x = new double[count];
            y = new double[count];
   ...
   //             Console.WriteLine(x[i] + "\t" + y[i]);
        ...
        public static double[] LatLonToOffsetYawList(double[] oldy, double[] oldx, double angleOff,double disOff, out double[] x, out double[] y, out double[] heading )
        {
            //int zone = (int)((lon[0] + 180.0) / 6) + 1;
            //FIXMe， 济青统一117度计算，优点是统一utm空间，缺点是青岛附近120度，误差大
            double t = UTMCentralMeridian(zoneJQ);
            double offa;
            int count = oldx.Length;
            x = new double[count];
            y = new double[count];
            heading = new double[count];
            double dx, dy, l, a = Math.PI / 180;
            for (int i = 0; i < count; i++)
            {
                MapLatLonToXY(oldy[i] * a, oldx[i] * a, t, out x[i], out y[i]);
            ...
            for (int i = 0; i < count; i++)
            {
                offa = (-heading[i] - angleOff + 90) * Math.PI / 180;

                x[i] = x[i] + disOff * Math.Cos(offa);
                y[i] = y[i] + disOff * Math.Sin(offa);
                UTMXYToLatLon(x[i], y[i], out y[i], out x[i], false);
            }
            return heading;
        }
        ...
        /// <param name="longout"></param>
        /// <param name="latout"></param>
        public static void LatLonOffsetUTMJQ(double[] x,double[] y, double[] yaw, double angleOff, double disOff,
            out double[] longout, out double[] latout)
        {
            double xd, yd, a;
            longout = new double[x.Length];
            latout = new double[x.Length];
            for (int i = 0; i< x.Length; i++)
            {
                a = (-yaw[i] - angleOff + 90) * Math.PI / 180;
                xd = x[i] + disOff * Math.Cos(a);
                yd = y[i] + disOff * Math.Sin(a);
                UTMXYToLatLon(xd, yd, out latout[i], out longout[i], false);
            }
